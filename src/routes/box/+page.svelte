<script lang="ts">
	...
	$: sb = Math.abs(Math.sin(alpha)) * b;
	$: ca = Math.cos(beta) * a;
	$: cb = Math.abs(Math.cos(alpha)) * b;
	$: totalWidth = ca + cb;

	$: betaR = beta + Math.PI / 2;
	$: alphaR = alpha + Math.PI / 2;

	$: lReachDown = +Math.sin(beta) * corrOreach;

	$: rReachDown = -sa + sb + Math.sin(alphaR) * w - Math.sin(betaR) * w + lReachDown;

	$: dshift = lReachDown + w - Math.sin(betaR) * w;

	$: horeach = Math.cos(betaR) * w - Math.cos(beta) * angledOreach;

	$: lBaseCutoff = Math.tan(0.5 * Math.PI - beta) * w;
	$: rBaseCutoff = Math.tan(0.5 * Math.PI - alpha) * w;
...
<container style="--pw: {(totalWidth / 1200).toFixed(4)}px;">
	...
	<div class="stats">
		...
		<span>
			overhang length: {angledOreach.toFixed(3)}
		</span>
		<span>
			cut from base left-side: {lBaseCutoff.toFixed(3)}
		</span>
		<span>
			cut from base right-side: {rBaseCutoff.toFixed(3)}
		</span>
		<span>
			left-side downward-overhang: {lReachDown.toFixed(3)}
		</span>
		<span>
			right-side downward-overhang: {rReachDown.toFixed(3)}
		</span>
	</div>

	<svg
		viewBox="{Math.min(0, horeach) * 1.2} -{totalHeight * 1.15} {totalWidth * 1.1} {totalHeight *
			1.3}"
		height="100%"
		width="100%"
	>
